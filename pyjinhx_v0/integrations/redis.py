...
- ``memory://`` — in-process FakeRedis (``pip install pyjinhx_v0[redis]``), for demos/tests
- ``redis://...`` — real Redis (``pip install pyjinhx_v0[redis]``)
"""
...
def _redis_clients(redis_url: str) -> tuple[Any, Any]:
    ...
    if redis_url == MEMORY_REDIS_URL or redis_url.startswith("memory://"):
        try:
            ...
            raise ImportError(
                "memory:// requires fakeredis: pip install pyjinhx_v0[redis]"
            ) from exc
    ...
    try:
        ...
        raise ImportError(
            "Redis URLs require the redis package: pip install pyjinhx_v0[redis]"
        ) from exc
...
class RedisInvalidationBackend(InvalidationBackend):
    ...
    def start(self, handler: Callable[[frozenset[str]], None]) -> None:
        ...
        self._thread = threading.Thread(
            ...
            name="pyjinhx_v0-redis-invalidation",
            daemon=True,
        )
        self._thread.start()
