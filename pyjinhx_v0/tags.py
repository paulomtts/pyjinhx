...
# Kept in sync with pyjinhx_v0.builtins.__all__ by a test. Listed here instead of
...
def _missing_template_error(tag_name: str) -> FileNotFoundError:
    if tag_name in _BUILTIN_TAG_NAMES:
        return FileNotFoundError(
            f"<{tag_name}> is a pyjinhx_v0 builtin but its class isn't registered. "
            f"Add `from pyjinhx_v0.builtins import {tag_name}` "
            f"(or `import pyjinhx_v0.builtins`) once at app startup so the tag resolves."
        )
