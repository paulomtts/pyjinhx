...
`pyjinhx/dev.py:23-55` is the only place in the codebase that already implements "raise if strict, else warn": `_DevConfig(enabled, strict)`, set by `enable_reactive_dev(strict=...)`, read by `_report()`, which raises `RuntimeError` when `_dev_config.strict` and logs a warning otherwise. That switch is reachable from configuration a project already has — `PjxSettings.reactive_dev` / the `PJX_REACTIVE_DEV` env var, applied through `config.configure_pyjinhx` -> `_apply_reactive_dev`. Routing this collision through the same switch means a project has exactly one knob controlling how loudly pyjinhx surfaces developer-error findings, instead of one knob plus one hardcoded always-raise site.
