...
| Request in flight, page navigation | `PJXPageLoader` | Automatic: htmx lifecycle events for GETs targeting `nav_targets`, plus any request from inside `[data-pjx-loader]`; `pjx.pageLoader.wrap(promise)` for manual async |
...
| `PJXPageLoader` | its own request ref-count across all tracked htmx requests (plus `pjx.pageLoader.wrap`) |
