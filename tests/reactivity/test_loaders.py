"""Browser contracts for PJXRegionLoader (ref-counting, resurrect, reduced motion)
and PJXPageLoader (htmx navigation tracking)."""
...
def test_resurrecting_show_fires_one_show_event(sink_page):
    sink_page.evaluate(
        ...
        document.getElementById('rx-region')
            .addEventListener('pjx:region-loader:show', () => window.__rlShows++);
        pjx.loader.region.show('rx-region');
        pjx.loader.region.hide('rx-region');
        pjx.loader.region.show('rx-region');
        """
    )
...
def test_reduced_motion_hide_uses_timer_fallback(browser, server_url):
    ...
    try:
        ...
        page.evaluate("pjx.loader.region.show('rx-region')")
