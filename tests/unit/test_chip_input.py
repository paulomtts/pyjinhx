...
"""Tests for the PJXChipInput builtin component."""
...
def _root(html: str) -> str:
    match = re.search(r"<div[^>]*pjx-chip-input[^>]*>", html)
    assert match, html
    return match.group(0)


def test_chip_input_renders_chips_for_each_value():
    html = str(PJXChipInput(id="ci1", name="tags", values=["alpha", "beta"]).render())
    # Each value should produce a hidden input
    hidden_inputs = re.findall(
        r'<input type="hidden" name="tags" value="([^"]+)"', html
    )
    assert hidden_inputs == ["alpha", "beta"], (
        f"Expected ['alpha', 'beta'], got {hidden_inputs}"
    )


def test_chip_input_hidden_inputs_have_correct_name():
    html = str(
        PJXChipInput(id="ci2", name="keywords", values=["foo", "bar", "baz"]).render()
    )
...
def test_chip_input_renders_remove_buttons_when_enabled():
    html = str(
        PJXChipInput(
            id="ci3", name="tags", values=["x", "y"], remove_label="Remove chip"
        ).render()
    )
...
def test_chip_input_disabled_no_text_field_no_remove_buttons():
    html = str(
        PJXChipInput(id="ci4", name="tags", values=["x"], disabled=True).render()
    )
    ...
    assert "data-pjx-chip-remove" not in markup, (
        "Disabled chip input should have no remove buttons in markup"
    )


def test_chip_input_disabled_hidden_inputs_still_present():
    html = str(
        PJXChipInput(id="ci5", name="tags", values=["x", "y"], disabled=True).render()
    )
    hidden_inputs = re.findall(
        r'<input type="hidden" name="tags" value="([^"]+)"', html
    )
    assert hidden_inputs == ["x", "y"], (
        "Hidden inputs must still be present when disabled"
    )


def test_chip_input_data_name_attr_on_root():
    html = str(PJXChipInput(id="ci6", name="skills", values=[]).render())
    root = _root(html)
...
def test_chip_input_data_remove_label_on_root():
    html = str(
        PJXChipInput(id="ci7", name="tags", values=[], remove_label="Delete").render()
    )
    root = _root(html)
...
def test_chip_input_extra_attrs_rendered():
    html = str(
        PJXChipInput(id="ci12", name="tags", extra_attrs={"data-test": "yes"}).render()
    )
    root = _root(html)
...
def test_chip_input_placeholder_in_text_field():
    html = str(PJXChipInput(id="ci13", name="tags", placeholder="Type here…").render())
    assert 'placeholder="Type here…"' in html


def test_chip_input_text_field_present_when_not_disabled():
    html = str(PJXChipInput(id="ci14", name="tags", values=[]).render())
    assert 'type="text"' in html


def test_chip_input_chip_value_order_preserved():
    values = ["c", "a", "b"]
    html = str(PJXChipInput(id="ci15", name="x", values=values).render())
