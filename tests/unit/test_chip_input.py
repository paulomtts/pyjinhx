...
def test_chip_input_hidden_inputs_have_correct_name():
    html = str(PJXChipInput(id="ci2", name="keywords", values=["foo", "bar", "baz"]).render())
    assert html.count('name="keywords"') >= 3  # text field + hidden inputs


def test_chip_input_renders_remove_buttons_when_enabled():
    html = str(PJXChipInput(id="ci3", name="tags", values=["x", "y"], remove_label="Remove chip").render())
...
def test_chip_input_disabled_no_text_field_no_remove_buttons():
    html = str(PJXChipInput(id="ci4", name="tags", values=["x"], disabled=True).render())
    ...
    assert 'data-pjx-chip-remove' not in markup, "Disabled chip input should have no remove buttons in markup"


def test_chip_input_disabled_hidden_inputs_still_present():
    html = str(PJXChipInput(id="ci5", name="tags", values=["x", "y"], disabled=True).render())
    hidden_inputs = re.findall(r'<input type="hidden" name="tags" value="([^"]+)"', html)
...
def test_chip_input_data_remove_label_on_root():
    html = str(PJXChipInput(id="ci7", name="tags", values=[], remove_label="Delete").render())
    root = _root(html)
...
def test_chip_input_extra_attrs_rendered():
    html = str(PJXChipInput(id="ci12", name="tags", extra_attrs={"data-test": "yes"}).render())
    root = _root(html)
