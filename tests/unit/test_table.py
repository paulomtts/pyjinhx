...
def test_table_modifier_classes():
    ...
    for mod in (
        "pjx-table--striped",
        "pjx-table--sticky",
        "pjx-table--density-compact",
        "pjx-table--bordered-all",
    ):
        assert mod in html
...
def test_header_cell_sort_direction_maps_to_aria_sort():
    asc = str(
        PJXTableHeaderCell(id="hc", sortable=True, sort="asc", content="N").render()
    )
    assert 'aria-sort="ascending"' in asc
    desc = str(
        PJXTableHeaderCell(id="hc", sortable=True, sort="desc", content="N").render()
    )
...
def test_row_selectable_prepends_checkbox_cell():
    html = str(
        PJXTableRow(id="r", selectable=True, value="42", content="<td>x</td>").render()
    )
...
def test_row_select_label_override_and_value_escaped():
    html = str(
        PJXTableRow(
            id="r", selectable=True, value='a"b', select_label="Pick", content=""
        ).render()
    )
