...
def test_class_name_and_extra_attrs_render():
    html = str(
        PJXBadge(
            id="b", label="x", class_name="mine", extra_attrs={"data-k": "v"}
        ).render()
    )
...
def test_extra_attrs_rejects_breakout_values():
    ...
    html = str(
        PJXBadge(id="b", label="x", extra_attrs={"data-k": '" onmouseover="x'}).render()
    )
    ...
    assert 'x-show="count &gt; 3"' in str(
        badge.render()
    ) or 'x-show="count > 3"' in str(badge.render())
...
def test_breadcrumb_aria_label_prop():
    html = str(
        PJXBreadcrumb(id="bc", items=[("Home", "/")], aria_label="Caminho").render()
    )
