...
def test_confirm_dialog_markers_and_labels():
    html = str(
        PJXConfirmDialog(
            id="cd", confirm_label="Confirmar", cancel_label="Cancelar"
        ).render()
    )
...
def test_confirm_dialog_contract():
    html = str(
        PJXConfirmDialog(
            id="cd", class_name="mine", extra_attrs={"data-k": "v"}
        ).render()
    )
...
def test_prompt_dialog_markers_and_labels():
    html = str(
        PJXPromptDialog(
            id="pd", input_label="Nome", submit_label="OK!", cancel_label="Voltar"
        ).render()
    )
...
def test_prompt_dialog_contract():
    html = str(
        PJXPromptDialog(
            id="pd", class_name="mine", extra_attrs={"data-k": "v"}
        ).render()
    )
