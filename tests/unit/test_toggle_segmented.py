...
from pyjinhx_v0.builtins import PJXSegmentedControl, PJXToggleSwitch

# ── PJXToggleSwitch ──────────────────────────────────────────────────────────────


def _toggle_root(html: str) -> str:
    match = re.search(r"<label[^>]*pjx-toggle-switch[^>]*>", html)
    assert match, html
    return match.group(0)


def test_toggle_switch_renders_checkbox():
    html = str(PJXToggleSwitch(id="ts1", name="active").render())
    assert 'type="checkbox"' in html
    assert 'name="active"' in html


def test_toggle_switch_value_attr():
    html = str(PJXToggleSwitch(id="ts2", name="n", value="yes").render())
    assert 'value="yes"' in html


def test_toggle_switch_checked_attr_when_true():
    html = str(PJXToggleSwitch(id="ts3", name="n", checked=True).render())
    assert " checked" in html


def test_toggle_switch_no_checked_attr_when_false():
    html = str(PJXToggleSwitch(id="ts4", name="n", checked=False).render())
    assert " checked" not in html


def test_toggle_switch_disabled_attr():
    html = str(PJXToggleSwitch(id="ts5", name="n", disabled=True).render())
    assert " disabled" in html


def test_toggle_switch_label_renders():
    html = str(PJXToggleSwitch(id="ts6", name="n", label="Dark mode").render())
    assert "Dark mode" in html
    assert "pjx-toggle-switch__label" in html


def test_toggle_switch_no_label_span_when_empty():
    html = str(PJXToggleSwitch(id="ts7", name="n", label="").render())
    markup = re.split(r"<style", html, maxsplit=1)[0]
    assert "pjx-toggle-switch__label" not in markup


def test_toggle_switch_track_and_thumb_present():
    html = str(PJXToggleSwitch(id="ts8", name="n").render())
...
def test_toggle_switch_no_js_file():
    ...
    from pyjinhx_v0.builtins import PJXToggleSwitch as _TS
...
def _seg_root(html: str) -> str:
    match = re.search(r"<div[^>]*pjx-segmented-control[^>]*>", html)
    assert match, html
    return match.group(0)
...
def test_segmented_control_no_js_file():
    ...
    from pyjinhx_v0.builtins import PJXSegmentedControl as _SC
...
def test_segmented_control_contract_fields():
    sc = PJXSegmentedControl(id="sc8", name="x")
