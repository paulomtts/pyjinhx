...
the gallery-demo test (which only validates ``<!-- demo: -->`` markers) cannot
see it. This test parses every ``PJXFoo(...)`` Python call and ``<PJXFoo ...>``
tag in the published docs and asserts each bare-word keyword/attribute is a
...
def _discover_builtins() -> dict[str, type[BaseComponent]]:
    """Builtin component classes keyed by their tag/class name.

    ``pyjinhx.builtins`` is a namespace of one package per component with no
    re-exports, so the classes have to be walked out of the submodules.
    """
    found: dict[str, type[BaseComponent]] = {}
    for module_info in pkgutil.walk_packages(b.__path__, b.__name__ + "."):
...
_BUILTINS: dict[str, type[BaseComponent]] = _discover_builtins()
...
def test_lint_has_something_to_lint():
    """Guards the lint against passing vacuously.

    The builtin discovery walks modules and the doc scan matches by class name,
    so a package reshuffle or a docs move could silently empty either side and
    turn the real assertion below into a no-op.
    """
    assert _BUILTINS, "no builtin components discovered under pyjinhx.builtins"
    linted = {
        m.group(1)
        for path in _DOC_FILES
        for pattern in (_PY_CALL, _TAG_OPEN)
        for m in pattern.finditer(path.read_text(encoding="utf-8"))
        if m.group(1) in _BUILTINS
    }
    assert linted, "docs contain no usage of any known builtin"
