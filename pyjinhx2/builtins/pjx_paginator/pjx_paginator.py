...
class PJXPaginator(BaseComponent):
    """A windowed pagination nav whose page links can drive HTMX swaps."""
    ...
    def _clamped_page(self) -> int:
        """The current page pulled back inside ``[1, total_pages]``."""
        return max(1, min(self.page, self.total_pages))

    def _href(self, n: int) -> str:
        """``url`` with its ``{page}`` placeholder filled in for page ``n``."""
    ...
    def _page_item(self, n: int, current: int) -> dict[str, Any]:
        """One page entry: a marker for the current page, a link otherwise."""
        if n == current:
    ...
    def _control(
        ...
        """One prev/next/first/last entry; disabled controls carry no href."""
        item: dict[str, Any] = {
            "kind": "control",
            "variant": variant,
            "label": label,
    ...
    def items(self) -> list[dict[str, Any]]:
        """The full render-ready sequence: first, prev, pages/ellipses, next, last."""
        ...
        for n in self._window():
            if prev_n is not None:
                # a gap of exactly one page is cheaper to show than to elide
                if n - prev_n == 2:
